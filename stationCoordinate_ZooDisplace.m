...
addpath 'C:\Users\Tz-Chian Chen\OneDrive - Florida State University\CalCOFI\output\'
zoovol = readtable('Anomaly_ZooDisplaceVol.csv',VariableNamingRule='preserve') % data generated in anomaly_ZooDisplace.m
...
zoovolnew.Properties.VariableNames=["Latitude","Longitude","Year","Month","Date","Season","Line","Station",...
        "ZooVol"];
    ...
    if tf == 0 % if match, select the period of onset< date < end
     extend(i,:)=array2table(zeros(1,10)); % indicated as no occurrence
        ...
        if height(tm)==0
           ...
           tf2=any(tm.st_Day<=table2array(zoovolnew(i,"Date")) & tm.end_Day>=table2array(zoovolnew(i,"Date"))); % select the occurrence event covering sampling time 
            if tf2 == 0 
            extend(i,:)=array2table(zeros(1,10)); % indicated as no occurrence 
            else % there is overlapping MHW event 
            overlap=tm(tm.st_Day<=table2array(zoovolnew(i,"Date")) & tm.end_Day>=table2array(zoovolnew(i,"Date")),:);
...
filename=['MHWOccurrence_ZooDisplace.csv'];
