...
%%% duration) and the anomaly of Vertically-integrated PP

%%%%% Required functions, table and raw data
% Net primary production data from in situ sampling data: https://calcofi.org/data/oceanographic-data/bottle-database/
...
ppnew.Properties.VariableNames=["Latitude","Longitude","Year","Month","Day","Season","Line","Station","PP"];
    ...
    if tf == 0 % if match, select the period of onset< date < end
     extend(i,:)=array2table(zeros(1,10)); % indicated as no occurrence
        ...
        if height(tm)==0
           ...
           tf2=any(tm.st_Day<=table2array(ppnew(i,"Day")) & tm.end_Day>=table2array(ppnew(i,"Day"))); % select the occurrence event covering sampling time 
            if tf2 == 0 
                extend(i,:)=array2table(zeros(1,10)); % indicated as no occurrence 
            else % there is overlapping MHW event 
                overlap=tm(tm.st_Day<=table2array(ppnew(i,"Day")) & tm.end_Day>=table2array(ppnew(i,"Day")),:);
...
for i=1:height(ppfinal)
    ppfinal.datetime(i,:)=datetime([ppfinal.Year(i),ppfinal.Month(i),ppfinal.Day(i)]);
end

% pick out the sampling points that co-occurr with MHW
sfmhw= ppfinal(ppfinal.occurrence==1,:);
...
% export data
cd ('...')
filename='OriFinal_MHW_PP.csv'  % manually type file name
