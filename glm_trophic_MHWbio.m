...
zoo = readtable("Final_MHW_ZooDisplace.csv","VariableNamingRule","preserve") % ZoopDisplace
sizephyto = readtable("Final_MHW_SizeFraction_v2.csv","VariableNamingRule","preserve") % SizeFraction
trapechla = readtable('Final_MHW_Chla_trape.csv','VariableNamingRule','preserve') % Bottle Chla
fucox = readtable("Final_MHW_HPLC.csv","VariableNamingRule","preserve") % fucoxanthin 
zoosatechla = readtable("Trophic_ZooDisplace_satelliteChla.csv","VariableNamingRule","preserve") % ZoopDisplace-satelliteChla, generated at COPAS hpcomputer
...
elseif j==2  % IntChla
    ...
    idx = find(target.datetime(i)==zoo.datetime&target.Line(i)==zoo.Line&target.Station(i)==zoo.Station); % same date & station
  if tf==0
    trophic(i,:)= [target.Year(i), target.Month(i), target.Date(i), target.Line(i), target.Station(i),NaN,NaN,target.Chla(i),NaN]
  else
    trophic(i,:)= [target.Year(i), target.Month(i), target.Date(i), target.Line(i), target.Station(i),...
...
writetable(finalpara,"result_Trophic.csv") 
